...
JButton addButton , cancelButton , checkOutButton ,searchButton ,
backButton ,
 exitButton ;
JComboBox comboBox;
...
// panel2 view rooms
panel2 = new JPanel();
panel2.setBorder(BorderFactory.createTitledBorder("view rooms"));
panel2.setBounds(400 , 80 , 350 , 420 ) ;
panel2.setLayout(null);
contentPane.add(panel2 );
// tools of panel1
jLabel2 = new JLabel("Room No. : ");
...
// radio
radio1 = new JRadioButton("Regular Room");
radio1. setBounds(25 , 330 , 120 , 30 ) ;
panel2.add(radio1 );
radio1.addActionListener(this);
radio2 = new JRadioButton("Suite Room");
radio2. setBounds(155 , 330 , 120 , 30 ) ;
panel2.add(radio2 );
radio2.addActionListener(this);
radio3 = new JRadioButton("Royal Suite");
radio3. setBounds(25 , 370 , 120 , 30 ) ;
panel2.add(radio3 );
radio3.addActionListener(this);
radio4 = new JRadioButton("Available Room");
...
if( event.getSource().equals((JButton) addButton)){
 // add reservation
 int RoomNo ;
 try{
 RoomNo = Integer.parseInt(text1.getText() ) ;
 } catch(NumberFormatException ex){
 JOptionPane.showMessageDialog(null , "room number must be only digit , try again");
 return ;
 }

Room roomObj = TestHotel.hotel.searchRoom(RoomNo) ;
if( roomObj == null || roomObj.isAvailable() == false){
System.out.println("this room is not available, try again");
return ;
}
...
if( mob.length() != 10 || !mob.startsWith("05"))
 throw new invalidMopileNo("mobile number must starts with 05 and only 10 digit");
...
JOptionPane.showMessageDialog(this , "room number must be only digit , try again");
return ;
}
String id = text2.getText() ;

if( TestHotel.hotel.cancelReservation(id, RoomNo))
...
String id = text2.getText() ;
Reservation res = TestHotel.hotel.searchReservation(id, RoomNo) ;
if(res == null )
...
//=============================================
if( event.getSource().equals((JRadioButton) radio1) &&
radio1.isSelected()){// regular
textArea.setText("");
RegularRoom[] list = TestHotel.hotel.getRegular();
for( int i = 0 ;i < list.length ; i++ )
if( list[i] != null )
textArea.append(list[i].toString()+ "\n");
}if( event.getSource().equals((JRadioButton) radio2) &&
radio2.isSelected()){ //suite
textArea.setText("");
Suite[] list2 = TestHotel.hotel.getAllSuite();
for( int i = 0 ;i < list2.length ; i++ )
if( list2[i] != null && ! (list2[i] instanceof RoyalSuite ))
textArea.append(list2[i].toString()+ "\n");
}
//=============================================
if( event.getSource().equals((JRadioButton) radio3) && radio3.isSelected()){ // royal
textArea.setText("");
int num ;
 try{
...
//=============================================
if( event.getSource().equals((JRadioButton) radio4) && radio4.isSelected()){ // available
textArea.setText("");
Room[] list4 = TestHotel.hotel.getAllAvailable();
